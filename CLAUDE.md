...
The system uses **SQLiteResultWriter** as the sole backend for storing tracking data:

- **SQLiteResultWriter** (Default)
    - Stores data in local SQLite databases (`/ethoscope_data/results/.../*.db`)
    - No additional setup required
    - Backed up via rsync (unified backup service) to node server
    - Best performance and reliability

- **dbAppender**
    - Appends data to an existing SQLite database
