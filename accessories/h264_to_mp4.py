...
def get_video_fps(video_file, user_fps=None):
    ...
    try:
        ...
        if not (1 <= fps <= 120):
            raise ValueError(
                f"Auto-detected FPS {fps} is out of acceptable range (1-120)"
            )
...
def print_progress(current_file, total_files, total_size, current_size):
    ...
    os.sys.stdout.write(
        f"\rMerging file {current_file}/{total_files} |{bar}| {percent:.2f}% Completed"
    )
    os.sys.stdout.flush()
...
def crawl(root_path, extension="h264", force=False, user_fps=None):
    ...
    have_mp4s = [p for p in all_folders if glob(os.path.join(p, "*.mp4"))]
    terminal_folders = [
        p for p in all_folders if glob(os.path.join(p, f"*.{extension}"))
    ]

    folders_to_process = (
        terminal_folders
        if force
        else [folder for folder in terminal_folders if folder not in have_mp4s]
    )
...
def purge_h264_files(root_path, extension="h264"):
    ...
    print(
        f"Found {total_folders} folders with .mp4 files. Proceeding to delete .{extension} files."
    )
    ...
    for idx, folder in enumerate(folders_with_mp4, start=1):
        ...
        if h264_files:
            print(
                f"[{idx}/{total_folders}] Deleting {len(h264_files)} .{extension} files in folder: {folder}"
            )
            ...
            print(
                f"[{idx}/{total_folders}] No .{extension} files found in folder: {folder}"
            )
...
if __name__ == "__main__":
    ...
    default_videos_path = os.getenv(
        "ETHOSCOPE_VIDEOS_DIR",
        "/home/murun/ethoscope/src/ethoscope/ethoscope/tests/static_files/videos/tmp",
    )
    ...
    parser.add_option(
        ...
        help=f"The root path containing the videos to process (default: {default_videos_path})",
    )
    parser.add_option(
        "-l",
        "--list",
        dest="list",
        default=False,
        help="Returns a list of folders containing mp4 files",
        action="store_true",
    )
    parser.add_option(
        "-e",
        ...
        dest="extension",
        default="h264",
        help="The extension of the video file chunks generated by the ethoscope",
    )
    parser.add_option(
        "--force",
        dest="force",
        default=False,
        help="Force recreating videos even when MP4s are present",
        action="store_true",
    )
    parser.add_option(
        "--fps",
        dest="fps",
        type="float",
        help="Override the auto-detection of FPS with a user-defined value",
    )
    parser.add_option(
        "--purge",
        dest="purge",
        default=False,
        help="Purge .h264 files in folders where .mp4 exists",
        action="store_true",
    )
