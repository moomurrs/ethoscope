(function () {
    var experimentsController = function (
        $scope,
        $http,
        $timeout,
        $routeParams,
        ...
        $scope.getBackupInfo = function () {
            return $http.get("/backup/summary");
        };
        ...
        $http.get("/runs_list").then(function (response) {
            var data = response.data;
            $scope.runs = data;
            $scope.totalRuns = 0;
            ...
            $scope
                ...
                .then(function (response) {
                    ...
                    for (var ix in $scope.runs) {
                        ...
                        if ($scope.runs[ix].end_time != "") {
                            $scope.runs[ix].end_time = $scope.runs[ix].end_time
                                .split(".")[0]
                                .replace(/-/g, "/");
                        }
                        $scope.runs[ix].start_time = $scope.runs[ix][
                            "start_time"
                        ]
                            .split(".")[0]
                            .replace(/-/g, "/");
                        ...
                        if (backup_info) {
                            ...
                            var db_name = $scope.runs[ix].experimental_data
                                .split("\\")
                                .pop()
                                .split("/")
                                .pop();
                            ...
                            // Backup type detection – SQLite only
                            if (backup_info.backup_types.sqlite.available) {
                                backup_type = "sqlite";
                            }
                            ...
                            if (
                                relevant_backup.available &&
                                relevant_backup.last_backup
                            ) {
                                $scope.runs[ix].last_backup =
                                    relevant_backup.last_backup;
