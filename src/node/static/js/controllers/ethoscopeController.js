    ...
    app.controller(
        ...
        function (
            ...
            function loadNodeData() {
                ...
                if (
                    ...
                    $scope.node.device_options =
                        nodeConfigCache.data.device_options;
                ...
                $http
                    ...
                    .then(function (response) {
                        ...
                        if (data.data) {
                            ...
                            $scope.isActive =
                                $scope.device.name.split("_").pop() !== "000";
                            ...
                            var timestamp =
                                Math.floor(new Date().getTime() / 30000.0) * 30;
                            $scope.device.url_img =
                                "/device/" +
                                $scope.device.id +
                                "/last_img?" +
                                timestamp;
                            $scope.device.url_stream =
                    ...
                    .catch(function (error) {
                        console.error(
                            "Failed to load critical device data:",
                            error,
                        );
                        // Fallback to individual requests if batch fails
            ...
            function loadNonCriticalDeviceData() {
                    ...
                    .then(function (response) {
                        ...
                        if (
                            ...
                            $http
                                ...
                                .then(function (modResponse) {
                                    ...
                                    $http
                                        ...
                                        .then(function (fwResponse) {
                                            $scope.firmware_status =
                                                fwResponse.data;
                                        })
                                        .catch(function (error) {
                                            console.warn(
                                                "Failed to load firmware status:",
                                                error,
                                            );
                                        });
                                })
                                .catch(function (error) {
                                    console.warn(
                                        "Failed to load module capabilities:",
                                        error,
                                    );
                                });
                        }
                ...
                $http
                    ...
                    .then(function (response) {
                        ...
                        $scope.can_stream =
                            typeof userOptions.streaming !== "undefined";
                        ...
                        if (
                            ...
                            // SQLite is the only result writer – hide selection and auto-select it
            ...
            $scope.getSelectedStimulatorOption = function (name) {
                return ethoscopeFormService.getSelectedStimulatorOption(
                    name,
                    $scope,
                );
            };
            ...
            $scope.getStimulatorArguments = function (className) {
                return ethoscopeFormService.getStimulatorArguments(
                    className,
                    $scope,
                );
            };
            ...
            $scope.removeStimulatorFromSequence = function (index) {
                ethoscopeFormService.removeStimulatorFromSequence(
                    index,
                    $scope,
                );
            };
            ...
            $scope.getInteractorOptionByName = function (name) {
                return ethoscopeFormService.getInteractorOptionByName(
                    name,
                    $scope,
                );
            };
            ...
            $scope.isArgumentVisible = function (arg, currentArgValues) {
                return ethoscopeFormService.isArgumentVisible(
                    arg,
                    currentArgValues,
                );
            };
            ...
            function loadBackupInfo(forceLoad) {
                ethoscopeBackupService.loadBackupInfo(
                    device_id,
                    $scope,
                    forceLoad,
                );
            ...
            $scope.getBackupStatusClass = function () {
                ...
                if (
                    ...
                    if (
                        recommendedType === "sqlite" ||
                        recommendedType === "rsync"
                    ) {
                ...
                // Legacy fallback
                if (
                    $scope.backupSummary.backedUp ===
                        $scope.backupSummary.total &&
            ...
            $scope.ethoscope.elapsedtime = function (t) {
                ...
                var minutes = Math.floor(
                    (elapsed - days * 86400 - hours * 3600) / 60,
                );
                var secs = Math.floor(
            ...
            $scope.ethoscope.start_tracking = function (option) {
                ...
                for (var opt in option) {
                    for (var arg in option[opt].arguments) {
                        ...
                        if (
                            ...
                            option[opt].arguments[arg].hasOwnProperty(
                                "formatted",
                            )
                        ) {
                            option[opt].arguments[arg] =
                                option[opt].arguments[arg].formatted;
                        }
                    }
                }
                ...
                if (
                    ...
                    var selectedIncubatorName =
                        option.experimental_info.arguments.location;
                    option.experimental_info.arguments.sensor =
                        $scope.get_ip_of_sensor(selectedIncubatorName);
                    ...
                    if (
                        ...
                        if (incubator.lights_on && incubator.lights_off) {
                            ...
                            option.experimental_info.arguments.light_period_minutes =
                                parseInt(incubator.light_period_minutes, 10) ||
                                1440;
                ...
                if (
                    ...
                    $scope.stimulatorSequence.length > 0
                ) {
                        ...
                        if (stimulator.arguments) {
                            for (var argName in stimulator.arguments) {
                                ...
                                if (
                                    ...
                                    typeof stimulator.arguments[argName] ===
                                        "object" &&
                                    stimulator.arguments[
                                        argName
                                    ].hasOwnProperty("formatted")
                                ) {
                                    stimulator.arguments[argName] =
                    ...
                    if ($scope.stimulatorSequence.length === 1) {
                        ...
                        if (singleStimulator.arguments) {
                            for (var key in singleStimulator.arguments) {
                                stimulatorArguments[key] =
                                    singleStimulator.arguments[key];
                            }
                        }
                ...
                if (
                    ...
                    isCustomTemplate =
                        ...
                        selectedTemplateInfo.type === "custom";
                    ...
                    if (isCustomTemplate && templateName) {
                        ...
                        $scope.uploadTemplateToDevice(
                            templateName,
                            function (success) {
                                if (success) {
                                    console.log(
                                        "Custom template uploaded successfully",
                                    );
                                } else {
                                    console.log(
                                        "Custom template upload failed, proceeding anyway",
                                    );
                                }
                        ...
                        $http
                            .post(
                                "/device/" + device_id + "/controls/start",
                                option,
                            )
                            .then(function (response) {
                    ...
                    console.log(
                        "Starting tracking with options (no template):",
                        option,
                    );
            ...
            $scope.getSelectedTemplateInfo = function (templateName) {
                if ($scope.available_templates) {
                    for (
                        var i = 0;
                        i < $scope.available_templates.length;
                        i++
                    ) {
                        if (
                            $scope.available_templates[i].value === templateName
                        ) {
            ...
            $scope.loadRoiTemplates = function () {
                $http.get("/roi_templates").then(
                    function (response) {
                        ...
                        if (
                            ...
                            for (
                                var i = 0;
                                i <
                                $scope.user_options.tracking.roi_builder.length;
                                i++
                            ) {
                                var roiBuilder =
                                    $scope.user_options.tracking.roi_builder[i];
                    ...
                    function (error) {
                        console.log(
                            "Could not load ROI templates from node server",
                        );
                    },
                );
            };
            ...
            $scope.applyDefaultTemplate = function () {
                ...
                var roiBuilderName =
                    $scope.selected_options.tracking.roi_builder.name;
                if (
                    !roiBuilderName ||
                ...
                $http
                    ...
                    .then(
                        function (response) {
                            var data = response.data;
                            console.log(
                                "Template uploaded to device: " + templateName,
                            );
                            if (callback) callback(true);
                        },
                        function (error) {
                            console.log(
                                "Failed to upload template to device: " +
                                    templateName,
                            );
            ...
            $scope.ethoscope.start_recording = function (option) {
                ...
                for (var opt in option) {
                    for (var arg in option[opt].arguments) {
                        if (
                            ...
                            option[opt].arguments[arg] =
                                option[opt].arguments[arg][1]; // Use timestamp
                        }
                    }
                }
                ...
                $http
                    .post(
                        "/device/" + device_id + "/controls/start_record",
                        option,
                    )
                    .then(function (response) {
                        $scope.device.status = response.data.status;
            ...
            $scope.ethoscope.backup = function () {
                ...
                $http
                    ...
                    .then(function (response) {
                        $scope.backupConfig = {
                            type:
                                response.data.recommended_backup_type || "none",
                            backupDatabases: true,
                            backupVideos: false,
                            loading: false,
                        };
            ...
            $scope.ethoscope.SQLdump = function () {
                function checkDumpStatus() {
                    $http
                        ...
                        .catch(function (error) {
                            console.error(
                                "Failed to check SQL dump status:",
                                error,
                            );
                        });
                }
                ...
                $http
                    ...
                    .then(function (response) {
                        ...
                        if (response.data.status === "updated") {
                            $http
                                .get(
                                    "/device/" + device_id + "/firmware/status",
                                )
                                .then(function (fwResponse) {
            ...
            function updateTimestampDisplay(deviceData) {
                ...
                if (deviceData.current_timestamp) {
                    ...
                    $scope.device_datetime =
                        $scope.device_timestamp.toLocaleString();

                    var local_time = new Date();
                    $scope.node_datetime = local_time.toLocaleString();
                    $scope.delta_t_min = Math.abs(
                        (local_time.getTime() / 1000 -
                            deviceData.current_timestamp) /
                            60,
                    );
                    ...
                    if ($scope.delta_t_min > 3 && $attempt < 3) {
                        ...
                        $attempt++;
                        console.log(
                            "Auto-correcting device time. Attempt:",
                            $attempt,
                        );
                    }
                } else {
            ...
            function refresh() {
                ...
                $http
                    ...
                    .then(function (response) {
                        ...
                        console.log(
                            "DEBUG: Data received in refresh function:",
                            data,
                        );
            ...
            function updateNodeDatabaseList() {
                ...
                if ($scope.device && $scope.device.databases) {
                    ...
                    console.log(
                        "No databases found on device, setting empty array",
                    );
                }
            }
            ...
            $scope.refreshDatabaseStatus = function () {
                ...
                $http
                    ...
                    .catch(function (error) {
                        ...
                        $scope.database_status.error =
                            "Failed to refresh database information: " +
                            error.data;
                        console.error(
                            "Failed to refresh database status:",
                            error,
                        );
                    });
            };
            ...
            function loadEnhancedVideoInfo() {
                $http
                    ...
                    .then(function (response) {
                        ...
                        var deviceData =
                            rsyncData.devices && rsyncData.devices[device_id];
                        ...
                        if (
                            ...
                            var videoFiles =
                                deviceData.transfer_details.videos.files || {};
                            var videoFileArray = [];
                    ...
                    .catch(function (error) {
                        console.log(
                            "Enhanced video info not available:",
                            error,
                        );
                    });
            }
            ...
            $scope.filterH264Files = function (videoFile) {
                return (
                    videoFile.is_h264 ||
                    videoFile.name.indexOf(
                        ".h264",
                        videoFile.name.length - 5,
                    ) !== -1
                );
            };
             ...
             */
            $scope.getVideoSegmentStyle = function (
                currentFile,
                allVideoFiles,
            ) {
                var h264Files = allVideoFiles.filter($scope.filterH264Files);
                var segmentWidth =
                    h264Files.length > 0 ? 100 / h264Files.length : 100;
            ...
            setTimeout(function () {
                ...
                $(document).on(
                    "click",
                    ".modal .option-list li strong",
                    function (e) {
                        e.preventDefault();
                        var $radioButton = $(this).siblings(
                            'input[type="radio"]',
                        );
            ...
            $scope.getReadableStimulatorName = function (className) {
                ...
                var nameMapping = {
                    ...
                    MiddleCrossingOdourStimulator:
                        "Middle Crossing Odour Stimulator",
                    MiddleCrossingOdourStimulatorFlushed:
                        "Flushed Odour Stimulator",
                };
