    ...
    app.factory("ethoscopeBackupService", function ($http) {
        return {
                ...
                return $http
                    ...
                    .then(
                        function (response) {
                            ...
                            console.log(
                                "DEBUG: Received backup data from endpoint:",
                                backupData,
                            );
                            ...
                            if (backupData.backup_status) {
                                ...
                                var backupTypes = JSON.parse(
                                    JSON.stringify(backupData.backup_status),
                                );
                                ...
                                if (
                                    ...
                                    for (var fileName in backupData.databases
                                        .SQLite) {
                                        if (
                                            backupData.databases.SQLite.hasOwnProperty(
                                                fileName,
                                            )
                                        ) {
                                            var fileInfo =
                                            ...
                                            var isRsyncEnhanced =
                                                fileInfo.rsync_enhanced ||
                                                false;
                                            var isFilesystemEnhanced =
                                                fileInfo.filesystem_enhanced ||
                                                false;
                                    ...
                                    console.log(
                                        "DEBUG: Created individual SQLite files structure:",
                                        sqliteFiles.length,
                                        "files",
                                    );
                                }
                                ...
                                if (
                                    backupData.video_files ||
                                    (backupData.backup_status &&
                                        backupData.backup_status.video) ||
                                    (backupData.databases &&
                                        backupData.databases.Video)
                                ) {
                                    var videoFileArray = [];
                                    ...
                                    if (
                                        backupData.databases &&
                                        backupData.databases.Video &&
                                        backupData.databases.Video.video_backup
                                    ) {
                                        var videoBackup =
                                            backupData.databases.Video
                                                .video_backup;
                                        ...
                                        console.log(
                                            "DEBUG: Processed video data from databases.Video:",
            ...
            updateBackupSummaryFromBackupInfo: function ($scope) {
                ...
                // Count available backup types (SQLite only)
            ...
            updateBackupSummaryFromLegacyData: function ($scope) {
                ...
                if ($scope.device.databases.SQLite) {
                    for (var dbName in $scope.device.databases.SQLite) {
                        if (
                            $scope.device.databases.SQLite.hasOwnProperty(
                                dbName,
                            )
                        ) {
                            total++;
